...
class Level2 {
    ...
    func edgeToLink(links: [Link],edge: any Edge) -> Link? {
        for l in links {
            if l.node_u == "\(edge.u)" && l.node_v == "\(edge.v)"{
                return l
            }
        }
        return nil
    ...
    func getClosestPointInSafeArea(position_u: (px:Float,py:Float), input_Graph: any Graph, percentage: Float, position_vertexes: [String : [String: Float]], links:[Link]) -> (px:Float,py:Float,px_return:Float,py_return:Float,edge:Link) {
        var result  : (px:Float,py:Float,px_return:Float,py_return:Float,edge:Link)? = nil
        ...
        for e in input_Graph.edgeList() {
            ...
            // TODO: MARK: PROBLEMA DELLA DISTANZA... AL MOMENTO È SOLO POSITIVA E QUINDI IL TARGET VIENE SEMPRE INDICATO A DESTRA, MENTRE SE FOSSE NEGATIVA SAREBBE A SINISTRA. QUINDI BISOGNA STABILIRE LA DIREZIONE ANCHE IN QUESTO CASO. FAI ATTENZIONE CHE d < distance_result NON DEVE CAMBIARE DI SIGNIFICATO E QUINDI DEVE DIVENTARE abs(d)<distance_result PERCHÈ RITORNO IL PUNTO A DISTANZA PIÙ PICCOLA.
            //print("DEBUG distance=\(d) _ dx=\(p!.dx) _ dy=\(p!.dy)")
            
            if( result==nil || d < distance_result){ // take the point at less distance
                result = (p!.x_point,p!.y_point,p2.p2x,p2.p2y,link_of_e)
                distance_result = d
                //result_debug = (p!.dx,p!.dy,p2.p2x,p2.p2y,distance_result,link_of_e)
            } // end if
        } // end for
    ...
    func getClosestEdge(position_u: (px:Float,py:Float), edges: [any Edge], percentage: Float, position_vertexes: [String : [String: Float]], links:[Link] ) -> (closestEdge:Link,distanceFromEdge:Float, x:Float, y:Float)  {
        var result : Link? = nil
    ...
    func getCurrentEdge(position_u: (px:Float,py:Float), edges: [any Edge], percentage: Float, position_vertexes: [String : [String: Float]], links:[Link] , nextNode: String) -> (currentEdge:Link?,distanceFromEdge:Float, x:Float, y:Float)  {
        ...
        var link_of_e : Link? = nil
        for e in edges {
            if "\(e.u)" == nextNode || "\(e.v)" == nextNode {
                for l in links {
                    print("nextnode",nextNode,"L",l,"edge",e)
                    if ((l.node_u == "\(e.u)" && l.node_v == "\(e.v)") || (l.node_u == "\(e.v)" &&   l.node_v == "\(e.u)")){
                        link_of_e = l
                        break
                    }
                }
                ...
                let d = distanceBetweenTwoPoints2D(p1x: position_u.px, p1y: position_u.py, p2x: p!.x_point, p2y: p!.y_point) // p2x: p2.p2x, p2y: p2.p2y)
                if( result==nil ||
                    d < distance_result){ // take the point at less distance
                    result = link_of_e!
                    distance_result = d
                    x_point = p!.x_point
                    y_point = p!.y_point
